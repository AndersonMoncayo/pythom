...
def home():
    if request.method == "POST":
        ...
        response, follow_up = find_response(user_message)
        historial = session.get('historial', [])
        historial.append({"pregunta": user_message, "respuesta": response, "follow_up": follow_up})
        session['historial'] = historial
        return jsonify({"respuesta": response, "follow_up": follow_up})
